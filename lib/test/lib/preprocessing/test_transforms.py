...
custom = Custom(lambda x: np.vstack((x, np.zeros((5, 2)))), name='AddZeros', desc='Add constant zero observations')

def test_custom_single_even():
    """Apply a custom transformation to a single even-length observation sequence"""
    assert_equal(custom(X_even), np.array([
...
def test_custom_single_odd():
    """Apply a custom transformation to a single odd-length observation sequence"""
    assert_equal(custom(X_odd), np.array([
...
def test_custom_multiple():
    """Apply a custom transformation to multiple observation sequences"""
    assert_all_equal(custom(Xs), [
        np.array([
