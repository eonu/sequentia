...
- | :math:`\Sigma_k^{(m)}` is a **covariance matrix** for the :math:`k^\text{th}` Gaussian component of the :math:`m^\text{th}` state, such that :math:`\Sigma_k^{(m)}\in\mathbb{R}^{D\times D}`.
